...
def calcRMSE(labelsAndPreds):
    ...
    """
    MSE = labelsAndPreds.map(lambda data : squaredError(data[0], data[1])).mean()
    return np.sqrt(MSE)
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, numIters, miniBatchFrac, regParam=reg, regType=regType, intercept=useIntercept)
...
def twoWayInteractions(lp):
    ...
    """
    new_features = []
    for e in itertools.product(lp.features, lp.features) :
        new_features.append(e[0]*e[1])
    new_features = np.array(new_features)
    return LabeledPoint(lp.label, np.hstack([lp.features, new_features]))
